...
run 명령으로 컨테이너가 실행(생성)되어 준비됨. 직접적인 프로세스를 실행하기 전까지는 활성화되지 않음. (마치 멈춘 기계같은 상태)  
컨테이너에서 프로세스를 실행하여 (exec명령 등) 가동해야만 활성화된 컨테이너가 됨. (켜진 기계의 상태가 됨)  

주로 쓰는 도커 실행 명령어 형식은 아래와 같음.  
```docker run -ti --gpus all --name=[컨테이너이름짓기] -v [밖경로:안경로] -p [밖포트:안포트] [도커이미지이름]```  
위 명령어의 옵션 설명 :
* ```-ti``` : cli 터미널을 의미. 첫 프로세스로 리눅스 기본쉘(eg. bash)이 실행되어 들어감. (exec 안해도 켜진 상태로 진입됨)  
* ```--gpus all``` : 가용 GPU 자원을 모두 사용함.
* ```-v``` : 스토리지 연결. 실제 (외부) 시스템 상 스토리지와 컨테이너 환경 안에서의 스토리지는 (가상화 원칙과 같이) "분리"되지만,  
...
컨테이너를 시작(부팅)하는 과정은 여러가지일 수 있으나, 대표적인 **시작 명령어 run과 start의 차이점**은 다음과 같음.  
* **run** : 지정된 이미지로부터 최초로 컨테이너를 생성하여 리눅스 시스템을 부팅하고, 준비되면 사용자에게 기능을 제공함. 
  * 부팅 후 준비되면 옵션으로 지정된 첫 프로세스를 실행함, ```-t``` 옵션(tty) 지정하면 사용자가 쉘(eg. bash)로 진입됨. 
  * 런명령에 ```--rm``` 옵션을 주는 경우, 부팅 첫 프로세스(eg. bash)가 종료되면 컨테이너도 종료되면서 스토리지도(볼륨) 제거되어, 모든 저장 내용이 사라지므로 매우 주의해야 함.  
    * 예를 들어 run -ti --rm 옵션으로 실행된 최초 bash에서 exit를 치면 컨테이너와 볼륨이 모두 제거됨. (반드시 알고 의도하는게 아니라면 run 명령어에 --rm 옵션을 주지 말것.)  
* **start** : 이미 부팅되어 있는 도커 프로세스를 시작하여 작업에 돌입하기 위한 명령어 (빠름). 

도커 상의 리눅스도 멀쩡히 멀티유저 멀티프로세스 OS이므로 아무 타이밍에나 서로 다른 id가 각기 로그인하여 동시다발적인 프로세스를 수행함.  
심지어 같은 사용자의 아이디마저 새로 bash를 띄워 로그인하면 별개의 프로세스로 로그인하여 분리된 프로세스를 실행 가능함.  

한 개의 리눅스 OS로서 부팅되어 있는 컨테이너에서, 명령어나 프로세스를 **실행하여 진입하는 명령어 exec와 attach 차이점**은 다음과 같음.  
* **exec** : 새로운 프로세스를 실행함. (bash를 exec하면 새로운 프로세스로써 bash가 뜨게 됨.) 
  ...
  * exec로 실행했던 내용 중 ram에서만 동작하던 내용들은 exit와 함께 사라질 것임. (램이 아니라 스토리지에 저장하는 작업들은 영원히 저장됨) 
* **attach** : 기존에 있던 프로세스에 재돌입 하여 앞으로 띄움(fg). 
